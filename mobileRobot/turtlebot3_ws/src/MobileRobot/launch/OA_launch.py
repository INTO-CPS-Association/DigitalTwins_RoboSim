...
def gazebo_world(context : LaunchContext):
    ...
    world_path = os.path.join(get_package_share_directory(package_name), 'worlds', 'RoboStar_world.world')
...
def group_action(context : LaunchContext):
    ...
    # if context.launch_configurations['onboard_calib'] == 'True':
    #     num_robots = 1
    ...
    sdf_path = os.path.join(get_package_share_directory(package_name), 'models', 'burger', 'model.sdf')
...
def generate_launch_description():
    ...
    ObstacleAvoidance = LaunchConfiguration('ObstacleAvoidance')
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(get_package_share_directory('MobileRobot'), 'maps', 'RoboStar_world.yaml'),
        description='Full path to map file to load')

    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(get_package_share_directory('MobileRobot'), 'params', 'nav2_params_namespaced.yaml'),
    ...
    declare_ObstacleAvoidance_cmd = DeclareLaunchArgument(
        'ObstacleAvoidance',
        default_value='False',
        description='Whether to run hand written Obstacle avoidance node')
    ...
    start_ObstacleAvoidance_cmd = Node(
        package='MobileRobot',
        executable='ObstacleAvoidance_real',
        output='screen',
        condition=IfCondition(
            PythonExpression([
                LaunchConfiguration('ObstacleAvoidance'),
                ' == True and ',
    ...
    ld.add_action(declare_ObstacleAvoidance_cmd)
    ...
    ld.add_action(start_ObstacleAvoidance_cmd)
