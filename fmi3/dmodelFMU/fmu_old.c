...
extern /*"C"*/fmi2Status fmi2GetReal(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, fmi2Real value[]) {
	...
	for (size_t i = 0; i < nvr; i++) {
		...
		value[i] = comp->fmiBuffer.realBuffer[vRef];
	}
...
extern /*"C"*/fmi2Status fmi2GetInteger(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, fmi2Integer value[]) {
	...
	for (size_t i = 0; i < nvr; i++) {
		fmi2ValueReference vRef = vr[i];
		value[i] = comp->fmiBuffer.intBuffer[vRef];
	}
...
extern /*"C"*/fmi2Status fmi2GetBoolean(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, fmi2Boolean value[]) {
	...
	for (size_t i = 0; i < nvr; i++) {
		fmi2ValueReference vRef = vr[i];
		value[i] = comp->fmiBuffer.booleanBuffer[vRef];
	}
...
extern /*"C"*/fmi2Status fmi2GetString(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, fmi2String value[]) {
	...
	for (size_t i = 0; i < nvr; i++) {
		fmi2ValueReference vRef = vr[i];
		value[i] = comp->fmiBuffer.stringBuffer[vRef];
	}
	return fmi2OK;
}
...
extern /*"C"*/fmi2Status fmi2SetReal(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2Real value[]) {
	...
	for (size_t i = 0; i < nvr; i++) {
		fmi2ValueReference vRef = vr[i];
		comp->fmiBuffer.realBuffer[vRef] = value[i];
		//printf("FMI real id=%d set to: %f\n", vRef, comp->fmiBuffer.realBuffer[vRef]);
	}

	return fmi2OK;
}

extern /*"C"*/fmi2Status fmi2SetInteger(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2Integer value[]) {
	ModelInstance* comp = (ModelInstance*) c;

	for (size_t i = 0; i < nvr; i++) {
		fmi2ValueReference vRef = vr[i];
		comp->fmiBuffer.intBuffer[vRef] = value[i];
        //printf("FMI real id=%d set to: %f\n", vRef, comp->fmiBuffer.intBuffer[vRef]);
	}

	return fmi2OK;
}

extern /*"C"*/fmi2Status fmi2SetBoolean(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2Boolean value[]) {
   ModelInstance* comp = (ModelInstance*) c;

    for (size_t i = 0; i < nvr; i++) {
    	fmi2ValueReference vRef = vr[i];
    	comp->fmiBuffer.booleanBuffer[vRef] = value[i];
		//printf("FMI real id=%d set to: %f\n", vRef, comp->fmiBuffer.booleanBuffer[vRef]);
	}

	return fmi2OK;
}

extern /*"C"*/fmi2Status fmi2SetString(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2String value[]) {
		ModelInstance* comp = (ModelInstance*) c;

		for (size_t i = 0; i < nvr; i++) {
			fmi2ValueReference vRef = vr[i];
			comp->fmiBuffer.stringBuffer[vRef] = value[i];
		//printf("FMI real id=%d set to: %f\n", vRef, comp->fmiBuffer.stringBuffer[vRef]);
	}
	return fmi2OK;
}
