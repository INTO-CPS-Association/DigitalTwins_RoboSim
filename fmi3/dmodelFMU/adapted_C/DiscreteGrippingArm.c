...
void __initialiseConcurrencyInfrastructure__(__Infrastructure__* state) {
	...
	start_DiscreteGrippingArm = (M_DiscreteGrippingArm_input_Enum_Channel*)malloc(sizeof(M_DiscreteGrippingArm_input_Enum_Channel));
	...
	end_DiscreteGrippingArm = (M_DiscreteGrippingArm_output_Enum_Channel*)malloc(sizeof(M_DiscreteGrippingArm_output_Enum_Channel));
	...
	start_cm = (C_cm_input_Enum_Channel*)malloc(sizeof(C_cm_input_Enum_Channel));
	pthread_barrier_init(&start_cm->can_read, NULL, 2);
	pthread_barrier_init(&start_cm->can_write, NULL, 2);
	end_cm = (C_cm_output_Enum_Channel*)malloc(sizeof(C_cm_output_Enum_Channel));
	pthread_barrier_init(&end_cm->can_read, NULL, 2);
	pthread_barrier_init(&end_cm->can_write, NULL, 2);
	start_sm = (sm_input_Enum_Channel*)malloc(sizeof(sm_input_Enum_Channel));
	pthread_barrier_init(&start_sm->can_read, NULL, 2);
	pthread_barrier_init(&start_sm->can_write, NULL, 2);
	end_sm = (sm_output_Enum_Channel*)malloc(sizeof(sm_output_Enum_Channel));
...
void setStartValues(ModelData *comp) {	
	...
	M(status) = "";
	M(done) = false;
	M(terminateSimulation) = false;
}


/***** *****/

void init(ModelData* comp) { //Replacement for the main function
		setStartValues(comp);
...
ModelData* tick(ModelData* comp){
	update_fmi_data(comp); // in interface.h
	{
		bool terminate__ = false;
