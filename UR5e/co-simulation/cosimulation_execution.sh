...
if [ "$using_rmqfmu" = true ] && [ "$using_simulation" = true ]; then
	...
	java -jar $maestro_path/$maestro_jar import sg1 multimodel_robot.json -output generate
elif [ "$using_rmqfmu" = false ] && [ "$using_simulation" = false ]; then
	java -jar $maestro_path/$maestro_jar import sg1 multimodel_normq_robot.json -output generate
elif [ "$using_rmqfmu" = false ] && [ "$using_simulation" = true ]; then
	java -jar $maestro_path/$maestro_jar import sg1 multimodel_normq_CoppeliaSim.json -output generate
fi
